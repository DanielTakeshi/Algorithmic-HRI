...
**Update**: I changed this code so that it only skips by 4. The reason is that spragnur's code will always skip by 4, so I want to ensure that the neural network classifier I train is going to have inputs that are consistent with each other. Note also that due to (1), we will never be in a case where we see a phi which may have two frames from the end of an episode and two frames from the next episode. See [this issue](https://github.com/DanielTakeshi/Algorithmic-HRI/issues/2) for more details.
...
This is for testing only, by the way. The training is done in a similar manner, except the results are not printed out. And, of course, the weights get updated in training.

In each row for the results file, we have the following:

- The first number is the epoch. In supervised learning, an epoch is one full pass over the dataset, but in reinforcement learning, an epoch is ambiguous. In his code, spragnur defined it as 10000 "testing steps" (in the NIPS version), so we'll just refer to an epoch as consisting of some total number of steps. The `ale_experiment.py` code gets called to run the experiment using the `run` method. That method contains a call to a `run_epoch` method (followed by a testing epoch) with a while loop that executes until we've exceeded the number of steps. The while loop calls its `run_episode` method with two parameters: the number of steps left, and whether it's testing/training, and the `run_episode` method initializes the episode and then keeps playing it (calls its own `_step` method) until the max steps has exceeded (or more commonly, if the ALE indicates it's game over).
- The second number is the number of episodes. This is straightforward, play one episode until it's game over, then we play again since in general the number of steps allocated per epoch is much larger than the number of steps it takes to play one game. However, as the epochs go on, we should be playing *fewer* episodes per epoch, since each episode should last longer, which intuitively means the agent is doing a better job. In the example above, I don't know why epoch 2 has 34 episodes and *zero* reward compared to epoch 1 with 63 episodes and 62 reward total. Maybe the agent is spending a lot of time steps not firing? I'm not sure. TODO Check?
- The third number is the total reward. We add up the rewards for each of the time steps in these episodes. So if episode 1 gave 10, episode 2 gave 20, we'd have 30 (if those were the only two in the epoch). The last episode does not get counted (unless it was the only episode, in which case STEPS_PER_TEST needs to be increased). 
- The fourth is reward_per_epoch, but that's a MISTAKE, it's reward_per_EPISODE. E.g. above we have 62/63=0.98412, which matches the first line above. This value is probably one that I want to care about the most! Again, the last testing episode does not get counted (unless it was the only episode, in which case STEPS_PER_TEST needs to be increased).
- The fifth is the mean q-value observed. This was recommended by DeepMind as a smoother way of evaluating the agent.

Other notes: after every *training* epoch, we save the network file in pickle format. Before and after every *testing* epoch, we call `ale_agent.start_testing()` and `ale_agent.finish_testing()`, respectively. The former method initializes counters, and the latter is what officially updates the two csv files, and it also holds out some data to compute average Q-values. The other csv file contains information about the loss and epsilon, but it's not as important. Oh, and about epsilon, it's updated at the granularity of steps, not epochs. Doubling the number of epochs but halving the steps per (training) epoch mean that the epsilon is decayed at the same rate for both. It should be a linear decay to one million, but I suppose I could increase it if I want to allow for more of the human data to work.
...
- Double check the parameters! In particular, the number of steps, whether we're using the human data or not, the model file for the human network, etc.
- Don't "vim" into the log files while the code is running! If you save by mistake, the files can't be written to anymore by the code. Use the "cat" or "head" commands to see the progress.
...
- What about increasing the number of steps for testing? Instead of 10000, we can double it to 20000, so we get a better sense of the average reward per epoch? **UPDATE**: The NATURE script does this.
...
- To make the game non-deterministic, use MAX_START_NULL_OPS to be some non-zero value (e.g. say 20), but this will probably be game-dependent. For Breakout, it's OK to set to this to be higher, but Pong may need this to be smaller. Actually wait, for Breakout I don't think it matters at all. **UPDATE**: The NATURE script does this.
